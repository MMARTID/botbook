...
This can be verified independently of the app's own DB by querying the provider's API directly with the account's API key and checking the registered URL against what's actually live: Retell `GET /get-agent/:id` and `GET /get-retell-llm/:id`, Telnyx `GET https://api.telnyx.com/v2/ai/assistants/:assistantId` (look at `tools[].webhook.url`; the native `hangup` tool has no URL). Telnyx also has a tool tester that hits the real webhook without placing a call: `POST /v2/ai/assistants/:assistantId/tools/:toolId/test` with `arguments` and `dynamic_variables`, which returns `{ status_code, response, success }`.
