    ...
    if (provider === 'outlook') {
      ...
      try {
        ...
        const event = await createMicrosoftCalendarEvent({
          ...
          reminderMinutesBeforeStart: buildImmediateReminderMinutes(startTime) ?? REMINDER_MINUTES_BEFORE_START,
        });
    ...
    const event = {
      ...
      reminders: {
        ...
        overrides: Array.from(
          new Map(
            [
              { method: 'popup', minutes: buildImmediateReminderMinutes(startTime) },
              { method: 'popup', minutes: REMINDER_MINUTES_BEFORE_START },
            ]
              .filter((reminder): reminder is { method: string; minutes: number } => reminder.minutes !== null)
              .map((reminder) => [`${reminder.method}:${reminder.minutes}`, reminder])
          ).values()
        ),
      },
    };
