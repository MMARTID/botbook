 ...
 */
function describeSelection({ production, hasSession }: { production: boolean; hasSession: boolean }) {
  if (production) return "Registro por invitación mientras terminamos el desarrollo.";
  if (hasSession) return "Añade una tarjeta: no se cobra nada hasta que termina la prueba.";
...
export function PlanSelectionLink({ planId, planName, featured, preselected = false }: PlanSelectionLinkProps) {
  ...
  // Solo se conoce en el navegador (localStorage): se resuelve tras montar
  // para que el HTML servido y el primer render coincidan.
  ...
  return (
    <>
      ...
      <p id={noteId} className={`mt-3 text-xs leading-5 ${featured ? "text-white/65" : "text-[#52525b]"}`}>
        {describeSelection({ production, hasSession })}
      </p>
      {bubble}
    </>
  );
