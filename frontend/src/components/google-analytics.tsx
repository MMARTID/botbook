...
function leerConsentimiento(): boolean | null {
  const valor = document.cookie.split("; ").find((cookie) => cookie.startsWith(`${NOMBRE_COOKIE}=`))?.split("=")[1];
...
export function GoogleAnalytics() {
  ...
  return (
    <>
      ...
      {consentimiento === null ? <aside className="fixed inset-x-4 bottom-4 z-50 mx-auto max-w-xl rounded-2xl border border-[#ddd6fe] bg-white p-5 shadow-[0_16px_40px_rgba(0,0,0,0.16)]" aria-label="Preferencias de cookies"><p className="text-sm font-semibold text-[#0a0a0a]">¿Nos ayudas a mejorar Alhabla?</p><p className="mt-1 text-sm leading-6 text-[#52525b]">Usamos Google Analytics solo con tu permiso para entender cómo se usa la aplicación.</p><div className="mt-4 flex flex-wrap gap-3"><button type="button" onClick={() => decidir(true)} className="btn-primary px-4 py-2 text-sm">Aceptar analítica</button><button type="button" onClick={() => decidir(false)} className="btn-secondary px-4 py-2 text-sm">Rechazar</button></div></aside> : null}
    </>
  );
}
